// @thesvg/icons - Platzi
// Auto-generated. Do not edit.

export const slug = "platzi";
export const title = "Platzi";
export const hex = "98CA3F";
export const categories = ["Education", "Platform"];
export const aliases = [];
export const svg = `<svg style="color:#98ca3f" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <path d="M10.64 1.13 2.49 9.28a3.84 3.84 0 0 0 0 5.44l8.15 8.15a3.84 3.84 0 0 0 5.44 0l2.72-2.71-2.72-2.72-2.72 2.72L5.2 12l8.16-8.16 5.44 5.44-5.44 5.44 2.72 2.72L21.5 12a3.84 3.84 0 0 0 0-5.44l-5.44-5.43a3.83 3.83 0 0 0-5.43 0z" fill="#98ca3f"/>
</svg>`;
export const variants = {
  "default": `<svg style="color:#98ca3f" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <path d="M10.64 1.13 2.49 9.28a3.84 3.84 0 0 0 0 5.44l8.15 8.15a3.84 3.84 0 0 0 5.44 0l2.72-2.71-2.72-2.72-2.72 2.72L5.2 12l8.16-8.16 5.44 5.44-5.44 5.44 2.72 2.72L21.5 12a3.84 3.84 0 0 0 0-5.44l-5.44-5.43a3.83 3.83 0 0 0-5.43 0z" fill="#98ca3f"/>
</svg>`,
  "mono": `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Platzi</title><path d="M10.6392 1.127L2.486 9.282a3.842 3.842 0 000 5.4361l8.1552 8.1552a3.842 3.842 0 005.4361 0l2.719-2.718-2.719-2.7181-2.718 2.718-8.1562-8.1551 8.1552-8.1552 5.437 5.4371-5.437 5.4361 2.718 2.719 5.4371-5.437a3.842 3.842 0 000-5.4372l-5.448-5.436a3.828 3.828 0 00-5.4252 0z"/></svg>`
};
export const license = "CC0-1.0";
export const url = "https://platzi.com/";

const platzi = { slug, title, hex, categories, aliases, svg, variants, license, url };
export default platzi;
