// @thesvg/icons - McDonald's
// Auto-generated. Do not edit.

export const slug = "mcdonalds";
export const title = "McDonald's";
export const hex = "FBC817";
export const categories = ["Food", "Platform"];
export const aliases = ["McDonalds"];
export const svg = `<?xml version="1.0"?>
<svg xmlns="http://www.w3.org/2000/svg" height="238.5" width="272.7" viewBox="0 0 272.70001 238.5">
 <path fill="#fc0" d="m195.8 17.933c23.3 0 42.2 98.3 42.2 219.7h34c0-130.7-34.3-236.5-76.3-236.5-24 0-45.2 31.7-59.2 81.5-14-49.8-35.2-81.5-59-81.5-42 0-76.2 105.7-76.2 236.4h34c0-121.4 18.7-219.6 42-219.6s42.2 90.8 42.2 202.8h33.8c0-112 19-202.8 42.3-202.8"/>
</svg>`;
export const variants = {
  "default": `<?xml version="1.0"?>
<svg xmlns="http://www.w3.org/2000/svg" height="238.5" width="272.7" viewBox="0 0 272.70001 238.5">
 <path fill="#fc0" d="m195.8 17.933c23.3 0 42.2 98.3 42.2 219.7h34c0-130.7-34.3-236.5-76.3-236.5-24 0-45.2 31.7-59.2 81.5-14-49.8-35.2-81.5-59-81.5-42 0-76.2 105.7-76.2 236.4h34c0-121.4 18.7-219.6 42-219.6s42.2 90.8 42.2 202.8h33.8c0-112 19-202.8 42.3-202.8"/>
</svg>`
};
export const license = "Fair Use";
export const url = "https://www.mcdonalds.com";

const mcdonalds = { slug, title, hex, categories, aliases, svg, variants, license, url };
export default mcdonalds;
